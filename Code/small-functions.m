intrinsic LinearlyEquivalentIn(D::DivCrvElt, S::SeqEnum[DivCrvElt]) -> BoolElt
  {Given a divisor D and a sequence of divisors S, check if D is linearly equivalent to a divisor in S.}

  for Dseen in S do
    if IsLinearlyEquivalent(D, Dseen) then
      return true, Dseen;
    end if;
  end for;
  return false;
...
intrinsic SmallFunctionsLinearlyEquivalent(Qs::SeqEnum[PlcCrvElt], d::RngIntElt) -> SeqEnum
  ...
  for Dden in Ds do
    for Dnum in Ds do
      D := Dden-Dnum;
      // if Degree(D) ne 1 then continue; end if;
      if D eq Parent(D)!0 then continue; end if;
      // check that D isn't linearly equivalent to a previously seen divisor
      if not LinearlyEquivalentIn(D, divisorsSeen) then
